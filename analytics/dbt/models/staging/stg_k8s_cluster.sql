...
temp AS (
    SELECT
        ...
        list_filter(
            node.conditions,
            x -> x.type = 'Ready'
        ) [1].lastHeartbeatTime :: timestamptz AS condition_ready_heartbeat_at,
        list_filter(
            node.conditions,
            x -> x.type = 'MemoryPressure'
        ) [1].status :: VARCHAR AS condition_memory_pressure_status,
        list_filter(
            node.conditions,
            x -> x.type = 'DiskPressure'
        ) [1].status :: VARCHAR AS condition_disk_pressure_status,
        list_filter(
            node.conditions,
            x -> x.type = 'PIDPressure'
        ) [1].status :: VARCHAR AS condition_pid_pressure_status,
        list_filter(
            node.conditions,
            x -> x.type = 'EtcdIsVoter'
        ) [1].status :: VARCHAR AS condition_etcd_is_voter_status,
...
SELECT
    ...
    COALESCE(
        node_control_plane = 'true',
        FALSE
    ) AS control_plane_enabled,
    COALESCE(
        node_etcd = 'true',
        FALSE
    ) AS node_etcd_enabled,
    ...
    COALESCE(
        condition_memory_pressure_status = 'True',
        FALSE
    ) AS memory_pressure,
    COALESCE(
        condition_disk_pressure_status = 'True',
        FALSE
    ) AS disk_pressure,
    COALESCE(
        condition_pid_pressure_status = 'True',
        FALSE
    ) AS pid_pressure,
    COALESCE(
        condition_etcd_is_voter_status = 'True',
        FALSE
    ) AS etcd_is_voter,
